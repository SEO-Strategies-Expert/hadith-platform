...
import { Field, TextArea, Checkbox, Select } from "@/components/admin/ui";
...
export function StageFields({
  ...
  s?: ProgramStage;
  courses: Array<{ id: string; titleAr: string; titleEn: string }>;
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <Select
          label="مقررات المرحلة"
          name="courseIds"
          options={courses.map((course) => ({
            value: course.id,
            label: `${course.titleAr} — ${course.titleEn}`,
          }))}
          defaultValue={selectedCourseIds}
          multiple
        />
        <p className="mt-1 text-[11.5px] text-ink-soft">
          يمكن اختيار عدة مقررات. تُربط بالمرحلة وتُحدّث قائمة المقررات العربية والإنجليزية تلقائيًا عند الحفظ.
        </p>
      </div>
      <div className="grid gap-5 sm:grid-cols-2">
