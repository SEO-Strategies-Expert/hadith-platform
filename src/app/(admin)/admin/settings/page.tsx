                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  {items.map((s) => {
                    ...
                    if (s.key === "font.heading" || s.key === "font.body") {
                      ...
                      const options = heading ? [["Thuluth", "Thuluth (الافتراضي)"], ["ThuluthAlt", "ThuluthAlt"], ["NaskhQ", "NaskhQ"], ["Cairo", "Cairo"], ["Tajawal", "Tajawal"], ["Amiri", "Amiri"], ["Scheherazade", "Scheherazade New"], ["NotoKufi", "Noto Kufi Arabic"], ["ReemKufi", "Reem Kufi"], ["Changa", "Changa"], ["Mada", "Mada"]] : [["PlexAr", "PlexAr / IBM Plex Sans Arabic (الافتراضي)"], ["Cairo", "Cairo"], ["Tajawal", "Tajawal"], ["Almarai", "Almarai"], ["NotoSans", "Noto Sans Arabic"], ["NotoKufi", "Noto Kufi Arabic"], ["Changa", "Changa"], ["Mada", "Mada"], ["Readex", "Readex Pro"], ["IBM", "IBM Plex Sans Arabic"], ["Segoe", "Segoe UI"], ["System", "System UI"]];
                      return <div key={s.key}><label className="mb-1.5 block text-[12px] font-bold text-navy-800">{label}</label><select name={s.key} defaultValue={value || (heading ? "Thuluth" : "PlexAr")} className="w-full rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-[14px] text-navy-950 outline-none focus:border-gold">{options.map(([fontValue, fontLabel]) => <option key={fontValue} value={fontValue}>{fontLabel}</option>)}</select><span className="mt-1 block text-[11px] text-ink-soft">يُطبّق على الواجهة الأمامية بعد الحفظ.</span></div>;
                    }
                    return (
