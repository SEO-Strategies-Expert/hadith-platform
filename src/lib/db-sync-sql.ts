...
/**
 * مزامنة عبر SQL مباشر — بديل لا يحتاج @prisma/engines (الذي يغيب في Vercel lambda).
 * يقرأ prisma/init.sql (المولَّد من `migrate diff --from-empty --to-schema-datamodel`)
 * وينفّذ كل عبارة مع تجاهل أخطاء "موجود مسبقًا".
 */
export async function syncViaSQL(): Promise<{ executed: number; skipped: number; errors: string[] }> {
  // prisma/init.sql مضمّن عبر outputFileTracingIncludes — نقرأه من FS فقط
  const candidates = [
    path.join(process.cwd(), "prisma", "init.sql"),
    "/var/task/prisma/init.sql",
  ];
  let sql = "";
  for (const p of candidates) {
    try {
      if (fs.existsSync(/* turbopackIgnore: true */ p)) {
        sql = fs.readFileSync(/* turbopackIgnore: true */ p, "utf-8");
        break;
      }
    } catch {}
  }
  if (!sql) {
    // fallback مضمّن — يعمل حتى لو لم يُنشر الملف عبر tracing
    sql = EMBEDDED_SQL;
  }

  //قسّم إلى عبارات منفصلة — كل عبارة تنتهي بـ ; ثم سطر جديد
  const rawStatements = sql
    .split(/;\s*\n/)
    .map((s) => s.trim())
    .filter((s) => s.length > 0 && !s.startsWith("warn ") && !s.startsWith("For more information"));

  // تجاهل أسطر التعليق التي ليست SQL فعّال
  ...
  for (const stmt of rawStatements) {
    ...
    // تخطّى عبارات فارغة أو تعليق فقط
    const withoutComments = withSemi
      .split("\n")
      .filter((l) => !l.trim().startsWith("--") || l.includes("CREATE") || l.includes("ALTER"))
      .join("\n")
      .trim();
    // إذا كانت العبارة فقط تعليق، تخطّها
    if (/^--\s*(Create|Alter|Add)/m.test(withSemi) || withSemi.includes("CREATE") || withSemi.includes("ALTER")) {
      statements.push(withSemi);
    } else if (withSemi.includes("CREATE") || withSemi.includes("ALTER") || withSemi.includes("SCHEMA")) {
      statements.push(withSemi);
    }
  }
