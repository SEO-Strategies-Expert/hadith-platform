...
import { claimCourseCertificate, createForumTopic, sendCourseMessage } from "@/app/(site)/student-lms-actions";
import { BookOpen, ClipboardCheck, FileText, Radio, Video } from "lucide-react";

const T = {
  ar: { course:"مقرر دراسي", back:"العودة إلى مقرراتي", instructor:"يقدمه", progress:"تقدمك", complete:"مكتمل", curriculum:"محتوى المقرر", materials:"الانتقال إلى مواد المقرر", sections:"أقسام", lessons:"دروس", continue:"متابعة التعلم", start:"ابدأ التعلم", certificate:"الحصول على الشهادة", preview:"معاينة", empty:"لم يُضف محتوى لهذا المقرر بعد.", min:"د", about:"عن هذا المقرر", kinds:{VIDEO:"فيديو",PDF:"ملف",TEXT:"قراءة",LIVE:"مباشر",QUIZ:"اختبار"} as Record<string,string> },
  en: { course:"Course", back:"Back to my courses", instructor:"Created by", progress:"Your progress", complete:"complete", curriculum:"Course content", materials:"Go to course materials", sections:"sections", lessons:"lectures", continue:"Continue learning", start:"Start course", certificate:"Get certificate", preview:"Preview", empty:"No course content has been added yet.", min:"min", about:"About this course", kinds:{VIDEO:"Video",PDF:"File",TEXT:"Reading",LIVE:"Live",QUIZ:"Quiz"} as Record<string,string> },
} as const;
...
export async function StudentCourseView({ lang, courseId }: { lang: Lang; courseId: string }) {
  ...
  const lessonIcon = (kind: string) => kind === "VIDEO" ? <Video size={15} /> : kind === "PDF" ? <FileText size={15} /> : kind === "QUIZ" ? <ClipboardCheck size={15} /> : kind === "LIVE" ? <Radio size={15} /> : <BookOpen size={15} />;
  ...
  return <main id="main" className="learning-page">
      ...
      <div className="container learning-hero-grid">
        ...
        <aside className="learning-side-card">
          ...
          <div className="learning-side-body">
            ...
            {pct >= 100 ? <div className="learning-complete-actions"><form action={claimCourseCertificate.bind(null, courseId, lang)}><button className="learning-primary" type="submit">🏆 {t.certificate}</button></form><a className="learning-secondary" href="#course-materials">{t.materials} ↓</a></div> : nextLesson && <Link className="learning-primary" href={studentHref(lang, `/lesson/${nextLesson.id}`)}>{doneCount ? t.continue : t.start}</Link>}
          </div>
        </aside>
        </div>
      </div>
    ...
    <section className="learning-body"><div className="container learning-layout"><div className="learning-main">
      ...
      <div className="learning-curriculum-head" id="course-materials"><h2>{t.curriculum}</h2><span>{num(course.modules.length,lang)} {t.sections} · {num(lessons.length,lang)} {t.lessons}{duration ? ` · ${num(duration,lang)} ${t.min}` : ""}</span></div>
      {!course.modules.length ? <div className="learning-empty">{t.empty}</div> : <div className="learning-curriculum">
        {course.modules.map((module, mi) => { const moduleDone = module.lessons.filter(l => progress.get(l.id)).length; return <details key={module.id} open={mi === 0}>
          ...
          <div className="learning-lessons">{module.lessons.map((lesson, li) => { const done = progress.get(lesson.id) === true; return <Link key={lesson.id} href={studentHref(lang, `/lesson/${lesson.id}`)} className={done ? "is-done" : ""}>
            <span className="lesson-check">{done ? "✓" : "○"}</span><span className="lesson-type-icon" title={t.kinds[lesson.kind] ?? lesson.kind}>{lessonIcon(lesson.kind)}</span><span className="lesson-title"><b>{num(li+1,lang)}. {title(lang,lesson.titleAr,lesson.titleEn)}</b><small>{t.kinds[lesson.kind] ?? lesson.kind}{lesson.freePreview ? ` · ${t.preview}` : ""}</small></span><span className="lesson-duration">{lesson.durationMin ? `${num(lesson.durationMin,lang)} ${t.min}` : "›"}</span>
          </Link>; })}</div>
        </details>; })}
      </div>}
    ...
    </div><aside className="learning-body-side"><StudentQuizAssignmentLinks lang={lang} courseId={courseId} /></aside></div></section>
  </main>;
}
